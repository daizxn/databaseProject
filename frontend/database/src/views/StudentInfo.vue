      ...
      <el-table :data="studentsData" style="width: 100%">
        ...
        <el-table-column fixed="right" label="Operations" min-width="120">
          <template #default="scope">
            <el-button type="text" size="default" @click.prevent="updateRow(scope.row)"
              >编辑
            </el-button>
            <el-button
              type="text"
...
const getClassesData = async (departmentId: number) => {
  try {
    console.log('获取班级数据, departmentId:', departmentId)
...
const getStudentsData = async (studentParam: Record<string, unknown>) => {
  console.log('获取学生数据:', studentParam)
  try {
...
const updateRow = (row: STUDENT_FULL_INFO | undefined) => {
  if (row) studentsUpdateParam.value = { ...row }
  else {
...
const save = async (formRef: FormInstance | undefined) => {
  if (!formRef) return
    ...
    await formRef.validate((valid: boolean, fields?: Record<string, unknown>) => {
      if (valid) {
        console.log('submit!')
      } else {
    ...
    try {
      ...
      if (addFlag.value) {
        // 新增学生
        response = await request.post('/students/add', studentsUpdateParam.value)
      } else {
        // 更新学生信息
        response = await request.put('/students/updateByNumber', studentsUpdateParam.value)
      }
      if (response.code === '5022') {
        console.error('保存学生信息失败:', response.msg)
        ElMessageBox.error({
          title: '错误',
